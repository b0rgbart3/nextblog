          ...
          if (rightShifted)
          {
            ...
            if (right.length == 0) {
              ...
              if (leftShifted) {
                ...
                // console.log('Neither is zero');
                // console.log('comparing: ', left[0].title, ', with: ', right[0].title);
                if (left[0].title.localeCompare(right[0].title) < 0) {
                  //console.log('Left is smaller than right: ');
                  const leftShifted = left.shift();
                  if (leftShifted) {
                    merged.push(leftShifted)
                  }
                 // console.log('Left length ends at: ', left.length);
                }
                else {
                    if (right[0].title.localeCompare(left[0].title) < 0) {
                    //  console.log('Right is smaller than left: ');
                      const rightShifted = right.shift()
                      if (rightShifted)
                      {
                        merged.push(rightShifted)
                      }
                    //  console.log('Right length ends at: ', right.length);
                    }
                    else {
                        // console.log(left[0].title, '==', right[0].title);
                        const leftShifted = left.shift()
                        if (leftShifted) {
                            merged.push(leftShifted)
                        }
