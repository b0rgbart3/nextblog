...
export default function Read() {
    ...
    const grabData = useCallback(() => {
      ...
      useEffect(() => {
       ...
       const renderBody = useCallback((body: string) => {
        ...
        let myJSX = ''
        return  (<>
        {split.map((par, index)  => (<div className='par' key={index}>{par}</div>))}
        </>)
       }, []);
  ...
  return (
    ...
    <div className='reading'>
        ...
        <div onClick={nextStory} className='next-story'><div className='right-arrow-adjust'><div className='right-arrow'><NextArrow /></div></div></div>
        </div>
    </div>
